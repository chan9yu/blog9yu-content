---
title: "[WebRTC 박살내기] 데이터 채널 구조와 활용법"
description: "WebRTC의 RTCDataChannel을 이해하고, 채팅·파일 전송·게임 동기화까지 실시간 데이터 전송의 모든 것을 알아봅니다."
...
| 사용 사례       | ordered | maxRetransmits / maxPacketLifeTime | 설명                          |
| --------------- | ------- | ---------------------------------- | ----------------------------- |
| **채팅**        | true    | –                                  | 순서 중요, 완전 신뢰          |
| **파일 전송**   | true    | –                                  | 모든 조각 필요                |
| **게임 좌표**   | false   | `0`                                | 최신값만 중요(손실 허용)      |
| **센서 데이터** | false   | `maxPacketLifeTime: 100` (예시)    | 일정 시간 경과시 폐기         |
| **제어 명령**   | true    | `3`                                | 제한 재전송(타임아웃 후 포기) |
...
RTCDataChannel은 모던 브라우저에서 지원합니다. 다만 iOS는 백그라운드/저전력 정책의 영향을 받아 연결 유지가 제한될 수 있습니다.
...
지금까지 이론을 정리한 내용을 바탕으로 간단한 1:1 실시간 화상상담 서비스를 만들어볼 예정입니다.
끝까지 읽어주셔서 감사합니다!
